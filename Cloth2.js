// This version has different time steps for physics calculations and graphical presentation
//________ Global Variables _________

var physics_accuracy = 3;
var physics_time_step = 0.020;          // (Seconds) Higher values can lead to unstability of the point position equation!
var canvas_refresh_time_step = 0.020;    // (Seconds)
var mouse_influence_distance = 10;
var mouse_cutting_distance = 15;
var mesh_height_units = 30;
var mesh_width_units = 50;
var mesh_top_y = 20;
var resting_link_length = 10;
var link_tearing_length = 30;
var elastic_stiffness = 140;   // Higher values can lead to unstability of the point position equation!
var nonleanier_elasticity = 1.4;  // 1 is linear elasticity
var damping_factor = 0.8;    // 0 => highest loss , 1 => no loss
var gravity_acceleration = 10;  // (m/S^2)
var point_mass = 1; // (Kg)
...
window.onload = function () {
    ...
    mesh = new Mesh();
	canvas_to_physics_timing_ratio = Math.floor(canvas_refresh_time_step / physics_time_step);
...
function mesh_refresh_loop()
{
	...
	setTimeout(mesh_refresh_loop, 1000 * physics_time_step);  // To convert to milliseconds
}

function canvas_refresh_loop() {
		ctx.clearRect(0, 0, canvas.width, canvas.height);
		mesh.draw();
		setTimeout(canvas_refresh_loop, 1000 * canvas_refresh_time_step);  // To convert to milliseconds
}
...
Point.prototype.update_position = function () {
	...
	{
		this.speed_x = (this.x - this.previous_x) / physics_time_step;
		this.speed_y = (this.y - this.previous_y) / physics_time_step;
		...
		this.x = this.x + this.speed_x * damping_factor * physics_time_step + 0.5 * (this.acceleration_x * Math.pow(physics_time_step,2));  // from physics x2-x1 = (x1-x0)*(t2-t1)*damping_factor/(t1-t0) + a*(t2-t1)^2/2   but t2-t1 = t1-t0 = physics_time_step!
		this.y = this.y + this.speed_y * damping_factor * physics_time_step + 0.5 * (this.acceleration_y * Math.pow(physics_time_step,2));
		this.force_x = 0;
		this.force_y = 0;
	}
};
